...
def plot_data_set1(data, ax, title, mean_styles, variance_styles):
    ...
    ax.plot(generation[:1000], NB_Mean[:1000], label='Theoretical Mean', **mean_styles['NB Mean'])
    ax.plot(generation[:1000], mean_TE_per_individual[:1000], label='Simulated Mean', **mean_styles['Simulated Mean'], alpha=0.9)
    ax.plot(generation[:1000], Simulated_Variance[:1000], label='Simulated Variance', **variance_styles['Simulated Variance'], alpha=0.9)
    ax.plot(generation[:1000], NB_Variance[:1000], label='Theoretical Variance', **variance_styles['NB Variance'])
    ...
    print(f"\n{title}:")
    print(f"Simulated Mean (final): {mean_TE_per_individual.iloc[-1]:.2f}")
    print(f"Simulated Mean (piecewise_average): {final_average_mean_TE:.2f}")
    print(f"NB Mean: {NB_Mean.iloc[-1]:.2f}")
    print(f"Simulated Variance (final): {Simulated_Variance.iloc[-1]:.2f}")
    print(f"Simulated Variance (piecewise_average): {final_average_simulated_variance:.2f}")
    print(f"NB Variance: {NB_Variance.iloc[-1]:.2f}")
...
def plot_data_set2(data, ax, title, mean_styles, variance_styles):
    ...
    ####### To calculate the final values for the simulated mean and variance (synergistic)
    t0 = 500
    k = 100
    ...
    ax.plot(generation[:1000], NB_Mean[:1000], label='Theoretical Mean', **mean_styles['NB Mean'])
    ax.plot(generation[:1000], mean_TE_per_individual[:1000], label='Simulated Mean', **mean_styles['Simulated Mean'], alpha=0.9)
    ax.plot(generation[:1000], Simulated_Variance[:1000], label='Simulated Variance', **variance_styles['Simulated Variance'],
            alpha=0.85)
    ax.plot(generation[:1000], NB_Variance[:1000], label='Theoretical Variance', **variance_styles['NB Variance'])
    ax.plot(generation[:1000], Charles_Mean[:1000], label='Charlesworth (1983) Mean', **variance_styles['Charles Mean'])
